...
async function main() {
  console.log("Seeding BuddyAi database...");

  // Default demo user for instant testing
  const demoEmail = "user@buddyai.local";
  let demoUser = await prisma.user.findUnique({
    where: { email: demoEmail },
  });

  if (!demoUser) {
    const passwordHash = await bcrypt.hash("buddy123", 10);
    ...
    console.log("Created demo user: user@buddyai.local / buddy123");
  }
